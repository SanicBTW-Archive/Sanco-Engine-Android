...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		if(ClientPrefs.optimizationType != EXTREME){
		...
		//heavy copy paste modifying each condition

		if(ClientPrefs.optimizationType == NONE)
		{
			add(gfGroup);
		...
		else if (ClientPrefs.optimizationType == BASIC)
		{
			// Shitty layering but whatev it works LOL
			if (curStage == 'limo')
				add(limo);
		...
		else if (ClientPrefs.optimizationType == ADVANCED)
		{
			add(dadGroup);
			add(boyfriendGroup);
		}
		else if (ClientPrefs.optimizationType == EXTREME)
		{
			//do nothing lol
		}
